...
def record_latency(response):
    ...
    slo_latency = duration_ms < 200
    slo_error = response.status_code < 500
    slo_available = response.status_code < 500
    conn = get_db_connection()
    conn.execute(
        "INSERT INTO latency (method, endpoint, status_code, duration_ms, slo_latency, slo_error, slo_available) VALUES (?, ?, ?, ?, ?, ?, ?)",
        (request.method, request.path, response.status_code, round(duration_ms, 3), slo_latency, slo_error, slo_available),
    )
    conn.commit()
    conn.close()
    return response
